...
**파생 관계(상위 문서)**: 어떤 문서가 다른 문서의 한 부분(비교표의 한 줄 등)에서 파생된 별도 문서일
땐 `_TEMPLATE.md`의 "상위 문서" 필드에 그 상위 문서 경로를 적습니다(예: `GT-Tonemapping.md`는
`Tone-Mapping.md`의 비교표 한 줄에서 파생됨). 세 보기 중 어느 걸 골라도, 이 관계가 있는 문서는
항상 상위 문서 바로 아래 들여써서(`↳`) 보입니다 — 그룹핑 축이 바뀌어도 파생 관계 자체는 그대로
유지되기 때문입니다. 단순 비교·배경지식 연결은 여기 말고 본문의 [같이 보기] 절에 남깁니다.
