...
`DevelopPrompt/VIEWER`의 엔진을 기반으로 한다. 다른 점 세 가지: `index.html`의 사이드바 그룹핑을
`ComputerScience/{분야}/` 카테고리 트리에 맞춘 것, 사이드바 위쪽에 **기술 스택별 / 프로젝트별 /
엔진별** 3-way 보기 토글을 추가한 것, 그리고 문서 메타 "상위 문서" 필드로 파생 관계가 있는 문서를
어느 보기에서든 상위 문서 바로 아래 들여써서 보여주는 것 — 셋 다 문서 메타 필드("관련 레포지토리"·
"엔진"·"상위 문서")에 의존하는 StudyProject만의 확장이라 `DevelopPrompt/VIEWER`로 역이식하지
않는다(그쪽 문서 컨벤션엔 그 필드들이 없다).
